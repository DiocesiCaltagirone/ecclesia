// frontend/src/components/sacramenti/FormCresima.jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { X } from 'lucide-react';

const FormCresima = ({ personaId, data, onClose, onSave }) => {
  const [formData, setFormData] = useState({
    data_cresima: data?.data_cresima?.split('T')[0] || '',
    volume: data?.volume || '',
    pagina: data?.pagina || '',
      ...
      <div className="bg-white rounded-lg w-full max-w-2xl shadow-2xl">
        ...
        <div className="bg-white border-b px-4 py-2.5 flex items-center justify-between">
          <h3 className="text-base font-bold text-gray-800">
            🔥 {data ? 'Modifica' : 'Aggiungi'} Cresima
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-3">
          ...
          <div className="bg-red-50 border border-red-200 rounded p-2.5">
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-red-500"
                />
              </div>
              <div>
            ...
            <div>
              <label className="block text-[11px] text-gray-600 mb-0.5">
                Data Cresima <span className="text-red-500">*</span>
              </label>
              <input
                type="date"
                required
                value={formData.data_cresima}
                onChange={(e) => setFormData({...formData, data_cresima: e.target.value})}
                className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-red-500"
              />
            </div>
          </div>
          ...
          {fuoriParrocchia ? (
            <div className="bg-orange-50 border border-orange-200 rounded p-2">
              <p className="text-xs text-orange-700 mb-2">
                ⚠️ Cresima celebrata fuori dalla parrocchia
              </p>
            ...
            <div className="bg-green-50 border border-green-200 rounded p-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-xs font-semibold text-green-800">Cresima nella tua parrocchia</p>
                  {enteCorrente && (
          ...
          <div>
            <label className="block text-xs font-medium text-gray-700 mb-1">Ministro</label>
            <input
              type="text"
              value={formData.ministro}
              onChange={(e) => setFormData({...formData, ministro: e.target.value})}
              className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-red-500"
              placeholder="Nome del Vescovo o Ministro"
            />
          </div>

          <div className="bg-purple-50 border border-purple-200 rounded p-2.5">
            <h4 className="text-xs font-semibold text-gray-700 mb-2">🤝 Padrini di Cresima</h4>
            <div className="grid grid-cols-2 gap-2">
              <div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-red-500 resize-none"
            />
          </div>
