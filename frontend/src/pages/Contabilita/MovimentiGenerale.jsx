...
const MovimentiGenerale = () => {
  ...
  useEffect(() => {
    applicaFiltri();
  }, [filtri, movimenti, ordineCrescente, contiNascosti]);
  ...
  const fetchData = async () => {
    try {
      ...
      const contiRes = await fetch('http://localhost:8000/api/contabilita/registri', { headers });
      const contiData = await contiRes.json();
      setConti(contiData || []);

      const catRes = await fetch('http://localhost:8000/api/contabilita/categorie', { headers });
      const catData = await catRes.json();
      setCategorie(catData.categorie || []);

      const movRes = await fetch('http://localhost:8000/api/contabilita/movimenti', { headers });
  ...
  const applicaFiltri = () => {
    ...
    if (filtri.contoId) {
      risultato = risultato.filter(m => m.registro_id === filtri.contoId);
    }
    ...
    // Calcola saldo progressivo PER SINGOLO CONTO
    const saldiPerConto = {};
    const saldiMap = {};
    risultatoOrdinatoCronologico.forEach(mov => {
      const contoId = mov.registro_id;

      if (saldiPerConto[contoId] === undefined) {
        saldiPerConto[contoId] = 0;
      }

      if (!mov.bloccato) {
        if (mov.tipo_movimento === 'entrata') {
          saldiPerConto[contoId] += parseFloat(mov.importo);
        } else {
          saldiPerConto[contoId] -= parseFloat(mov.importo);
        }
      }

      saldiMap[mov.id] = saldiPerConto[contoId];
    });
  ...
  const getNomeConto = (registroId) => {
    const conto = conti.find(c => c.id === registroId);
    return conto ? conto.nome : 'N/D';
  };
    ...
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-2 flex-shrink-0">
        <div className="flex items-center justify-between">
          <h1 className="text-base font-bold text-gray-800">💳 MOVIMENTAZIONE GENERALE</h1>
          <button
            onClick={() => openModal()}
        ...
        {showFiltri && (
          <div className="mt-2 grid grid-cols-6 gap-2">
            ...
            <select
              ...
              onChange={(e) => setFiltri({ ...filtri, contoId: e.target.value })}
              className="px-2 py-1 border border-gray-300 rounded text-xs"
            >
        ...
        <table className="w-full text-xs border-collapse">
          <thead className="bg-gray-50 border-b-2 border-gray-300 sticky top-0 z-10">
            <tr>
              ...
              <th className="text-left px-2 py-1.5 font-bold text-gray-700 border-r border-gray-200" style={{ width: '100px', maxWidth: '100px' }}>Conto</th>
              <th className="text-left px-2 py-1.5 font-bold text-gray-700 border-r border-gray-200" style={{ width: '250px', maxWidth: '250px' }}>Categoria</th>
              ...
              <th className="text-left px-2 py-1.5 font-bold text-gray-700 border-r border-gray-200" style={{ width: '180px', maxWidth: '180px' }}>Note</th>
            ...
            {movimentiFiltrati.length === 0 ? (
              ...
              movimentiFiltrati.map((mov) => (
                ...
                >
                  ...
                  <td className="px-2 py-1 text-gray-800 border-r border-gray-100 font-medium" style={{ width: '100px', maxWidth: '100px' }}>
                    <div className="truncate">{getNomeConto(mov.registro_id)}</div>
                  </td>
                  <td className="px-2 py-1 text-gray-800 border-r border-gray-100" style={{ width: '250px', maxWidth: '250px' }}>
                  ...
                  <td className="px-2 py-1 text-right text-red-600 font-semibold border-r border-gray-100 font-mono" style={{ width: '90px', maxWidth: '90px' }}>
                    {mov.tipo_movimento === 'uscita' ? formatCurrency(mov.importo) : ''}
                  </td>
                  <td className="px-2 py-1 text-right text-green-600 font-semibold border-r border-gray-100 font-mono" style={{ width: '100px', maxWidth: '100px' }}>
                    {mov.tipo_movimento === 'entrata' ? formatCurrency(mov.importo) : ''}
                  </td>
                  <td className={`px-2 py-1 text-right font-bold border-r border-gray-100 font-mono ${mov.saldo_progressivo >= 0 ? 'text-blue-700' : 'text-red-700'}`} style={{ width: '100px', maxWidth: '100px' }}>
                  ...
                  {mov.tipo_speciale === 'saldo_iniziale' ? (
                    <td colSpan="2" className="px-3 py-2 align-middle">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={contiNascosti.has(mov.registro_id)}
                          onChange={(e) => {
                            e.stopPropagation();
                            const nuoviNascosti = new Set(contiNascosti);
                            if (e.target.checked) {
                              nuoviNascosti.add(mov.registro_id);
                            } else {
                              nuoviNascosti.delete(mov.registro_id);
                            }
                            setContiNascosti(nuoviNascosti);
                          }}
                    ...
                    <>
                      <td className="px-2 py-1.5 text-gray-600 text-[10px] border-r border-gray-100 align-middle" style={{ width: '180px', maxWidth: '180px' }}>
...
export default MovimentiGenerale;